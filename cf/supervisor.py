...
#         total_loss = 0
#         for x, y in loader:
#             # print("x:", x.shape, "y:", y.shape)  # [16,8,113,8]
            
#             x = x.to(self.device)  # [B, T, N, D]
#             y = y.to(self.device)  # [B, 1, N, D]

#             # print(y.shape)  # [8,1,113,8] 预测一天的情况
...
#         self.log_fn(f"[Test] Macro F1: {macro_f1:.4f} | Micro F1: {micro_f1:.4f}")

#         # 保存预测结果
#         pickle.dump(y_true, open(os.path.join(self.output_dir, "labels.pkl"), "wb"))
#         pickle.dump(
#             y_pred_bin, open(os.path.join(self.output_dir, "predict.pkl"), "wb")
#         )

#     def save_model(self):
#         torch.save(
#             self.model.state_dict(), os.path.join(self.output_dir, "best_model.pt")
#         )
